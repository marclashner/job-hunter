...
from tests.support.evaluation import evaluation
...
def test_unwrap_evaluation_run_accepts_plain_evaluation() -> None:
    ...
    raw, usage = unwrap_evaluation_run(model)
    assert raw is model
...
def test_unwrap_evaluation_run_reads_usage() -> None:
    ...
    raw, extracted = unwrap_evaluation_run(EvaluationRunResult(evaluation=model, usage=usage))
    assert raw is model
